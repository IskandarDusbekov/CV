...
def to_credit_packages(apps, schema_editor):
    """Bir martalik "Bitta rezyume" tarifi o'rniga kredit paketlari."""
    PricingPlan = apps.get_model("users", "PricingPlan")
    PricingPlan.objects.filter(scope="cv").update(scope="credits", is_active=False)

    packs = [
        ("kredit-1", "1 ta rezyume", 1, Decimal("9000"), 1, False),
        ("kredit-3", "3 ta rezyume", 3, Decimal("19000"), 2, True),
        ("kredit-10", "10 ta rezyume", 10, Decimal("49000"), 3, False),
    ]
    for code, name, credits, price, order, featured in packs:
        PricingPlan.objects.update_or_create(code=code, defaults={
            ...
            "description": f"{credits} ta rezyumeni ochish: PDF + Word, muddatsiz",
...
def seed_site(apps, schema_editor):
    ...
    Page.objects.get_or_create(slug="biz-haqimizda", defaults={
        ...
        "sort_order": 1,
        "body": "tezrezyume.uz — O'zbekistondagi ish qidiruvchilar uchun yaratilgan AI rezyume yaratuvchi.\n\n"
                ...
                "ATS tizimlaridan o'tadigan rezyumega aylantiradi.\n\n"
